...
{
    // Convert totum:// to totum:
    //
    //    Cannot handle this later, because totum:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open totum.conf with the associated application */
...
{
    return GetAutostartDir() / "totum.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a totum.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the totum app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add totum app to startup item list
